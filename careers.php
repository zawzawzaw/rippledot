<?php 
  global $current_page;

  $current_page = "careers-page";
?>
<!doctype html>
<!--[if lt IE 7]>      <html class="no-js lt-ie9 lt-ie8 lt-ie7" lang=""> <![endif]-->
<!--[if IE 7]>         <html class="no-js lt-ie9 lt-ie8" lang=""> <![endif]-->
<!--[if IE 8]>         <html class="no-js lt-ie9" lang=""> <![endif]-->
<!--[if gt IE 8]><!--> <html class="no-js" lang=""> <!--<![endif]-->
<head>
  <?php include 'head.php'; ?>
</head>

<body class="page-careers">
  <?php include "header.php"; ?>


  <!--
     ___ _   _ _____ ____   ___
    |_ _| \ | |_   _|  _ \ / _ \
     | ||  \| | | | | |_) | | | |
     | || |\  | | | |  _ <| |_| |
    |___|_| \_| |_| |_| \_\\___/

  -->

  <article id="page-careers-intro-content-section">

    <div class="desktop-header-spacer"></div>
    <div class="desktop-header-subnav-spacer"></div>
    <div class="mobile-header-spacer"></div>

    <div class="container-fluid has-breakpoint">

      <div class="row">
        <div class="col-md-12 col-md-push-0 col-sm-10 col-sm-push-1 col-xs-12 col-xs-push-0">
          <div id="page-careers-intro-title">
            <div class="page-default-title">
              <h4>Careers</h4>
              <h1>Join Our Team</h1>
            </div>
          </div>
        </div>
      </div>

      <div class="row">
        <div class="col-md-12 col-md-push-0 col-sm-10 col-sm-push-1 col-xs-12 col-xs-push-0">

          <div class="row">
            <div class="col-md-6">

              

            </div>
          </div>  <!-- inner row -->

          <div class="row">
            <div class="col-md-6">

              <div id="page-careers-intro-content">

                <div id="page-careers-intro-introduction">
                  <div class="default-copy">
                    <p>Client-focused, results-oriented, lean, elite and collaborative: That’s the Rippledot team in a nutshell. <strong>Team</strong> is the operative word here.</p>
                  </div>
                </div>

                <div id="page-careers-intro-copy">
                  <div class="default-copy">
                    <p>“We don’t have a ‘soccer hero’ kind of culture,” says CEO Atin Kukreja. “We are more akin to a rugby team, a team with great instincts that works together in a high-stakes environment to get that ball across the line.”</p>
                    <p>Whether you’re joining us as an established professional or a recent graduate, expect to do meaningful work on a daily basis, diving deep with clients to thoroughly dissect their business and deeply understand their challenges and their strengths.</p>
                    <p>Expect success too. Our consultancy-based methods have a proven track record of successful transactions and high valuations in Southeast Asia and beyond.</p>
                  </div>
                </div>

              </div> <!-- page-careers-intro-content -->

            </div>
            <div class="col-md-4 col-md-push-1 col-sm-12 col-sm-push-0">



              <div id="page-careers-intro-cta-container">
                <div class="sans-container-fluid-mobile">
                  <div class="title">
                    <h3>Make an impact at Rippledot Capital</h3>
                  </div>
                  <div class="cta-container">
                    <a href="professionals.php" class="large-arrow-cta">Professionals</a>
                    <div class="cta-spacer"></div>
                    <a href="students-and-graduates.php" class="large-arrow-cta">Students & graduates</a>
                  </div>
                </div>
              </div> <!-- page-careers-intro-cta-container -->

            </div>
          </div> <!-- inner row -->

        </div>
      </div> <!-- outer row -->

    </div> <!-- container-fluid -->

  </article> <!-- page-careers-intro-content-section -->
  





  <!--
     ____  _____ ___  ____  _     _____
    |  _ \| ____/ _ \|  _ \| |   | ____|
    | |_) |  _|| | | | |_) | |   |  _|
    |  __/| |__| |_| |  __/| |___| |___
    |_|   |_____\___/|_|   |_____|_____|

  -->

  <article id="page-careers-people-content-section">

    <div id="page-careers-people-image-mobile" class="visible-sm visible-xs">
      <div class="manic-image-container" data-vertical-align="top">
        <img src="" 
          data-image-tablet="images_cms/careers/careers-people-02-tablet.jpg"
          data-image-mobile="images_cms/careers/careers-people-02-mobile.jpg">
      </div>
    </div>

    <div class="container-fluid has-breakpoint">
      <div class="row">


        <div class="col-md-4 col-tablet-landscape-5 hidden-sm hidden-xs">

          <div id="page-careers-people-image">
            <div class="manic-image-container" data-vertical-align="top">
              <img src="" data-image-desktop="images_cms/careers/careers-people-02.jpg">
            </div>
          </div>

        </div>

        <div class="col-md-7 col-md-push-1 col-sm-10 col-sm-push-1 col-xs-12 col-xs-push-0">

          <div id="page-careers-people-content">

            <div class="page-careers-people-center-container">
              <div class="page-careers-people-center">

                <div class="row">
                  <div class="col-md-8">


                    <div id="page-careers-people-title-02">
                      <h3>Brad Wong</h3>
                      <h4>Managing Director</h4>
                    </div>

                    <div id="page-careers-people-quote">
                      <p>
                        <span class="quote-start"></span>
                        Each Team Member brings extensive industry knowledge, unrivaled relationships and substantial deal experience, resulting in a team of the industry's most sought after advisors.
                        <span class="quote-end"></span>
                      </p>
                    </div>


                  </div>
                </div> <!-- inner row -->

              </div>
            </div>


          </div> <!-- page-careers-people-content -->


        </div>
      </div> <!-- first row -->
    </div>

  </article>






  <?php include "footer.php"; ?>
  <?php include "sidebar.php"; ?>
  <?php include "script.php" ?>
</body>
</html>